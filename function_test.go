...
func Test_funcReadDiscreteInputs(t *testing.T) {
	...
	tests := []struct {
		...
		{"数据长度data小于FuncReadMinSize", args{readReg, []byte{0, 0, 0}}, nil, true},
...
func Test_funcWriteSingleCoil(t *testing.T) {
	...
	tests := []struct {
		...
		{"数据长度data小于FuncReadMinSize", args{newNodeReg(), []byte{0, 0, 0}}, nil, true},
...
func Test_funcWriteMultiCoils(t *testing.T) {
	...
	tests := []struct {
		...
		{"数据长度data小于FuncWriteMultiMinSize", args{newNodeReg(), []byte{0, 0, 0}}, nil, true},
...
func Test_funcReadInputRegisters(t *testing.T) {
	...
	tests := []struct {
		...
		{"数据长度data小于FuncReadMinSize", args{readReg, []byte{0x00, 0x00, 0x00}}, nil, true},
...
func Test_funcWriteSingleRegister(t *testing.T) {
	...
	tests := []struct {
		...
		{"数据长度data小于FuncReadMinSize", args{newNodeReg(), []byte{0x00, 0x00, 0x00}}, nil, true},
...
func Test_funcWriteMultiHoldingRegisters(t *testing.T) {
	...
	tests := []struct {
		...
		{"数据长度data小于FuncWriteMultiMinSize", args{newNodeReg(), []byte{0x00, 0x00, 0x00}}, nil, true},
...
func Test_funcReadWriteMultiHoldingRegisters(t *testing.T) {
	...
	tests := []struct {
		...
		{"读数量小于1或大于125 或者 写数量小于1或大于121",
			args{newNodeReg(), []byte{0x00, 0x00, 0x00, 0x7e, 0x00, 0x00, 0x00, 0x7a, 0x00}}, nil, true},
		{"读写值", args{newNodeReg(), []byte{0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x02, 0x12, 0x34}}, []byte{0x02, 0x12, 0x34}, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func Test_funcMaskWriteRegisters(t *testing.T) {
	...
	tests := []struct {
		...
		{"数据长度data小于FuncMaskWriteMinSize", args{newNodeReg(), []byte{0x00, 0x00, 0x00}}, nil, true},
